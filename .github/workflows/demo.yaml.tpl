...
name: "FAST demo-django-setup stage"
  ...
  fast-pr:
    ...
    steps:
      ...
      - id: set-image
        ...
        run: |
          if [ "${{ github.event.pull_request.merged }}" = "true" ]; then
            echo "image=${{ env.REGION }}-docker.pkg.dev/${{ env.PROJECT_ID }}/${{ env.AR_REPO }}/${{ env.SERVICE_NAME }}:${{ github.sha }}" >> "$GITHUB_ENV"
          else
            echo "image=${{ env.REGION }}-docker.pkg.dev/${{ env.PROJECT_ID }}/${{ env.AR_REPO }}/${{ env.SERVICE_NAME }}:pr-${{ github.event.number }}" >> "$GITHUB_ENV"
          fi
      ...
      - id: tf-config-provider
        ...
        run: |
          gcloud storage cp -r \
            "gs://kh-prod-iac-core-0-iac-outputs/providers/${{env.provider_file}}" ./terraform/
          gcloud storage cp -r \
            "gs://kh-prod-iac-core-0-iac-outputs/tfvars/0-org-setup.auto.tfvars.json" ./terraform/
          gcloud storage cp -r \
            "gs://kh-prod-iac-core-0-iac-outputs/tfvars/0-globals.auto.tfvars.json" ./terraform/s
      ...
      - id: tf-bootstrap-ar
        if: github.event.pull_request.merged == true && success()
        name: Terraform apply (bootstrap Artifact Registry)
        run: |
          cd terraform
